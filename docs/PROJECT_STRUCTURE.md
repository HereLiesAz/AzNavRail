...
**Toolchain.** React Native 0.86, React 19, TypeScript 5.9, Jest 30, ESLint 9 (flat config in
`eslint.config.js`), `react-native-builder-bob` 0.43. `npm ci` resolves cleanly with no
`--legacy-peer-deps`.

**Build output.** `lib/` is generated by `npm run build` (Bob: commonjs + module + typescript
targets) and is **gitignored**. It is not committed — a stale committed `lib/` silently broke the
`sample-pwa` build for as long as `bob build` was failing. Anything that consumes the built package
(the Pages workflow, a local `sample-pwa` build) must run `npm run build` here first.

**Tests.** `npm test` runs two Jest projects declared in `jest.config.js`:

| Project | What it covers | Environment |
| --- | --- | --- |
| `native` | The React Native components, under the `react-native` preset | node |
| `web` | The plain-DOM components in `src/web`, with `react-dom` | jsdom |

They are separate projects because the RN preset's setup defines `window` on the global, which
jsdom has already defined and locked; one shared project fails outright. The RN suites run against
the **real** React Native component tree — a hand-rolled `jest.setup.js` mock used to stand in for
it, and it hid real defects (a bare `" "` text node inside a `<View>`, a dropdown that only opened
inside a `measureInWindow` callback).