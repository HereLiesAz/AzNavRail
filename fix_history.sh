...
internal object HistoryStore {
    ...
    // Used for tests to bypass the actual settings since SharedPreferences might fail without Context in tests
    var testSettingsOverride: (suspend (String, String) -> Unit)? = null
    var testSettingsReader: (suspend (String, String) -> String)? = null
    ...
    suspend fun addEntry(text: String, historyContext: String?) {
        ...
        mutex.withLock {
            ...
            list.remove(text)
            list.add(0, text)
            if (testSettingsOverride != null) {
                testSettingsOverride?.invoke("az_history_$ctx", Json.encodeToString(list))
            } else {
                try { azCacheSettings.putString("az_history_$ctx", Json.encodeToString(list)) } catch (e: Exception) {}
            }
        }
    }

    private suspend fun ensureLoaded(ctx: String) {
        if (loadedContexts.add(ctx)) {
            val saved = if (testSettingsReader != null) {
                testSettingsReader?.invoke("az_history_$ctx", "") ?: ""
            } else {
                try { azCacheSettings.getString("az_history_$ctx", "") } catch (e: Exception) { "" }
            }
            if (saved.isNotBlank()) {
                val list = histories.getOrPut(ctx) { mutableListOf() }
