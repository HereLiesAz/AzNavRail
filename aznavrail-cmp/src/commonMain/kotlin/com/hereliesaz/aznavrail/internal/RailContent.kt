...
internal fun RailContent(
    ...
    /** Whether this item currently wears the **tertiary** highlight (app-driven). */
 ...
 * ([RailItems]). Only non-host, direct-or-nested entries in [AzNavItem.nestedRailItems] are eligible
 * (a host child can still be tapped to expand/collapse inline, but never becomes the displayed
 * selection): falls back to the first non-host entry when [AzNavItem.selectedChildId] is unset or
 * names a host; returns null when [AzNavItem.reflectSelectionInParent] is false, this isn't a nested
 * rail, or the nested rail has no eligible children (the parent's own text/content is used instead).
 */
