 ...
 *   (edge order is a plain running sum over each rail's live measured size, so this falls out of
 *   ordinary recomposition rather than needing special-casing).
...
private fun FloatingDockGroup(
    ...
    LaunchedEffect(hosts.map { it.id }) {
        ...
        hosts.forEach { host ->
            ...
            val saved = AzUnattachedStore.loadFloating(host.id)
    ...
    fun persistDock(id: String, dock: FloatingDock, priority: Float) {
        AzUnattachedStore.saveFloating(id, AzFloatingSave(dock, priority, 0f))
    }
