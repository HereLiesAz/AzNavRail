...
/**
 * Which renderer draws the guidance engine's routed instructions. Same [AzStatusEngine], same
 * [AzEdge]/[AzGoal] graph, same [ResolvedInstruction]s — only the presentation changes.
 *
 * @see AzInstructionOverlay
 * @see AzEscortOverlay
 */
enum class AzGuidanceStyle {
    /**
     * The default: a thin outline around the target, a small card near it explaining the hop, and a
     * connector between them. Non-blocking and never dims the screen, but is still a caption — a
     * second element describing the first.
     */
    Callout,

    /**
     * [The framework's Escort](https://github.com/HereLiesAz/Conveyance/blob/main/docs/CONVEYANCE-FRAMEWORK.md#52-the-escort):
     * a breathing halo drawn directly on the target, that travels — animates its own position — from
     * wherever it was guiding a moment ago to wherever the next hop is. No separate card; the
     * instruction text is published as a live-region announcement instead of written on screen.
     */
    Escort,
}
