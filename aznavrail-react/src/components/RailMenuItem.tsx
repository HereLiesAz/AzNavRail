...
export const RailMenuItem: React.FC<RailMenuItemProps> = ({
  ...
  if (item.isToggle) {
    ...
    if (item.options && displayOption) {
      ...
      if (
        ...
        item.menuOptions &&
        idx >= 0 &&
        idx < item.menuOptions.length
      ) {
        displayText = item.menuOptions[idx];
      } else {
        if (item.menuOptions) {
          console.warn(
            `Cycler configuration mismatch for item='${item.text}': selectedOption='${displayOption}', index=${idx}, optionsSize=${item.options.length}, menuOptionsSize=${item.menuOptions.length}`
          );
        }
  ...
  if (item.isHost || item.isNestedRail)
    accessibilityState.expanded = isExpandedHost;
      ...
      <TouchableOpacity
        ...
        style={[
          styles.menuItem,
          {
            ...
            backgroundColor: !item.isHost
              ? `${item.fillColor ?? item.color ?? '#000000'}1F`
              : 'transparent',
          },
        ]}
