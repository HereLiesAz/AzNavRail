...
describe('paged routing', () => {
  it('shows the step at the cursor', () => {
    ...
    expect(f1.resolved[0].instruction.highlight).toEqual({
      type: 'Target',
      id: 'ball',
    });
  });
  ...
  it('keeps single-callout edges unchanged (stepTotal 1)', () => {
    const edge: AzEdge = {
      from: 'a',
      to: 'b',
      instruction: { text: 'Do A', highlight: { type: 'Item', id: 'ia' } },
    };
    const f = routeInstructions(
      [edge],
      { g: { id: 'g', target: 'b' } },
...
describe('highlight + shape resolution', () => {
  it('resolveAzHighlight precedence is target, selector, active token, item, none', () => {
    expect(resolveAzHighlight('item', () => 'sel', 't')).toEqual({
      type: 'Target',
      id: 't',
    });
    expect(resolveAzHighlight('item', () => 'sel', undefined).type).toBe(
      'Dynamic'
    );
    expect(resolveAzHighlight(AZ_ITEM_ACTIVE, undefined, undefined)).toEqual({
      type: 'ActiveItem',
    });
    expect(resolveAzHighlight('item', undefined, undefined)).toEqual({
      type: 'Item',
      id: 'item',
    });
    expect(resolveAzHighlight(undefined, undefined, undefined)).toEqual({
      type: 'None',
    });
  });
  ...
  it('resolveItemId folds ActiveItem and Dynamic against the live active id', () => {
    ...
    expect(
      resolveItemId({ type: 'Dynamic', selector: () => 'layer.1' }, null)
    ).toBe('layer.1');
    expect(
      resolveItemId({ type: 'Dynamic', selector: () => AZ_ITEM_ACTIVE }, 'home')
    ).toBe('home');
  ...
  it('resolveShape resolves targets, areas and items, degrading to null', () => {
    ...
    expect(
      resolveShape({ type: 'Target', id: 't' }, {}, null, targets)
    ).toEqual(circle);
    expect(
      resolveShape({ type: 'Target', id: 'gone' }, {}, null, targets)
    ).toBeNull();
    expect(
      resolveShape({ type: 'Target', id: 'missing' }, {}, null, targets)
    ).toBeNull();
    expect(
      resolveShape(
        { type: 'Area', left: 5, top: 6, width: 7, height: 8 },
        {},
        null,
        {}
      )?.type
    ...
    const cache: Record<string, AzItemBounds> = {
      home: { x: 0, y: 0, width: 10, height: 10 },
    };
    expect(
      resolveShape({ type: 'Item', id: 'home' }, cache, null, {})?.type
    ).toBe('Rect');
    expect(
      resolveShape({ type: 'Item', id: 'nope' }, cache, null, {})
    ).toBeNull();
    expect(resolveShape({ type: 'ActiveItem' }, cache, 'home', {})?.type).toBe(
      'Rect'
    );
  ...
  it('shapeBounds wraps each geometry with padding', () => {
    expect(
      shapeBounds({ type: 'Circle', cx: 100, cy: 50, radius: 10, padding: 5 })
    ).toEqual({ left: 85, top: 35, width: 30, height: 30 });
    expect(
      shapeBounds({ type: 'Rect', left: 10, top: 20, width: 30, height: 40 })
