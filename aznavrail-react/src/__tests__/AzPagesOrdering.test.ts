...
describe('pages ordering', () => {
  const bg = (id: string, weight: number, page: number) => ({
    id,
    weight,
    page,
    content: null,
  });
  const on = (id: string, page: number) => ({
    id,
    page,
    alignment: 'TopStart' as any,
    content: null,
  });
  ...
  it('falls back to legacy weight sort when pages disabled', () => {
    ...
    expect(orderBackgrounds(items, false).map((i) => i.weight)).toEqual([
      1, 10,
    ]);
  });
  ...
  it('keeps declaration order within a page (stable)', () => {
    ...
    expect(orderOnscreen(items, true).map((i) => i.id)).toEqual([
      'first',
      'second',
    ]);
  });
