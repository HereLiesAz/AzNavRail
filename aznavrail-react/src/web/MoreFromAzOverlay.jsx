...
export default function MoreFromAzOverlay({
  ...
  return (
    <div className="az-about-overlay" style={{ background: surface }}>
      ...
      {apps && apps.length === 0 && (
        <div className="az-about-empty">Couldn't load apps right now.</div>
      )}
      ...
      {current && (
        <div className="az-more-body">
          <div className="az-more-carousel">
            {apps.map((app, idx) => (
              <button
                ...
                style={{
                  borderColor: idx === selected ? accent : `${accent}66`,
                }}
          ...
          <div className="az-more-detail">
            ...
            {current.description && (
              <div className="az-more-desc">{current.description}</div>
            )}
            <div className="az-more-actions">
              {current.webUrl && (
                <a
                  className="az-about-repo"
                  href={current.webUrl}
                  target="_blank"
                  rel="noreferrer"
                  style={{ borderColor: accent, color: accent }}
                >
                  {current.isPwa ? 'Open' : 'Website'}
                </a>
              )}
              {current.playStoreUrl && (
                <a
                  className="az-about-repo"
                  href={current.playStoreUrl}
                  target="_blank"
                  rel="noreferrer"
                  style={{ borderColor: accent, color: accent }}
                >
