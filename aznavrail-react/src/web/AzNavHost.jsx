 ...
 */
const AzNavHost = ({ railProps = {}, background, children }) => {
  const settings = railProps.settings || {};
  const { dockingSide = 'LEFT', collapsedRailWidth = '100px' } = settings;
  ...
  const titlePaddingSide =
    dockingSide === 'LEFT' ? { paddingRight: '32px' } : { paddingLeft: '32px' };

  return (
    <div className="az-nav-host">
      {/* Layer 0: Background */}
      <div className="az-nav-host-background">{background}</div>
