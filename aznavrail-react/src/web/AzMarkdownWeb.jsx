...
function renderInline(text, accent, keyPrefix) {
  ...
  while ((m = INLINE.exec(text)) !== null) {
    ...
    if (m[2])
      nodes.push(
        <code key={`${keyPrefix}-${k}`} className="az-md-code">
          {m[2]}
        </code>
      );
    else if (m[4])
      nodes.push(<strong key={`${keyPrefix}-${k}`}>{m[4]}</strong>);
    else if (m[6])
      nodes.push(<strong key={`${keyPrefix}-${k}`}>{m[6]}</strong>);
...
export default function AzMarkdownWeb({ markdown, accent }) {
  ...
  const flushPara = () => {
    if (para.length) {
      ...
      if (text)
        blocks.push(
          <p key={key++} className="az-md-p">
            {renderInline(text, accent, `p${key}`)}
          </p>
        );
      para = [];
    }
  ...
  while (i < lines.length) {
    ...
    if (line.trimStart().startsWith('```')) {
      ...
      blocks.push(
        <pre key={key++} className="az-md-pre">
          <code>{code.join('\n')}</code>
        </pre>
      );
      ...
      flushPara();
      blocks.push(
        <hr key={key++} className="az-md-hr" style={{ borderColor: accent }} />
      );
      ...
      blocks.push(
        <Tag key={key++} className="az-md-h" style={{ color }}>
          {renderInline(line.replace(/^#+\s/, ''), accent, `h${key}`)}
        </Tag>
      );
      ...
      while (i < lines.length && /^\s*([-*+] |\d+\. )/.test(lines[i])) {
        ...
        items.push(
          <li key={items.length}>
            {renderInline(content, accent, `li${key}-${items.length}`)}
          </li>
        );
        i++;
      }
      ...
      blocks.push(
        ordered ? (
          <ol key={key++} className="az-md-list">
            {items}
          </ol>
        ) : (
          <ul key={key++} className="az-md-list">
            {items}
          </ul>
        )
      );
